...
Base::Base(UT::HighCmd cmd, UT::HighState state, ros::NodeHandle *nh, ros::NodeHandle *p_nh): robot_safety(UT::LeggedType::Go1) {
    ...
    set_body = nh_->advertiseService("set_body", &Base::setBodyCallback, this);
...
bool Base::setModeCallback(go1_legged_msgs::SetMode::Request &req, go1_legged_msgs::SetMode::Response &res) {
    ...
    robot_high_cmd.bodyHeight = req.body_height;
    res.success = true;
    return true;
}

bool Base::setBodyCallback(go1_legged_msgs::SetBody::Request &req, go1_legged_msgs::SetBody::Response &res) {
    robot_high_cmd.mode = req.mode;   // Hardcoded since euler poses are only used in mode 1 (standing)
    robot_high_cmd.bodyHeight = req.body_height;
    std::array<float, 3> euler = {req.yaw, req.pitch, req.roll};
    robot_high_cmd.euler = euler;
    res.success = true;
    return true;
}
