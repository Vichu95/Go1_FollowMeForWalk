...
void OpenManipulator::initOpenManipulator(bool using_actual_robot_state, STRING usb_port, STRING baud_rate, float control_loop_time)
{
  ...
  *****************************************************************************/
  ...
  addJoint("joint2",  // my name
           ...
           12,        // actuator id
           1.790,    // max joint limit
           -1.790);    // min joint limit
  ...
  addJoint("joint3",  // my name
           ...
           -1.790);  // min joint limit (-1.67 rad)
  ...
  addTool("gripper",  // my name
          ...
          math::convertRPYToRotationMatrix(0.0, 0.0, 0.0), // relative orientation
          15,         // actuator id
          0.019,      // max gripper limit (0.01)
          -0.010,     // min gripper limit (-0.01)
          -0.015);    // Change unit from `meter` to `radian`
          // Necessary so that sim & real can be similar
